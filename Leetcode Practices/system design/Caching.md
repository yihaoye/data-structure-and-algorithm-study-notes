...
缓存在几乎每个计算层级中都被使用：硬件、操作系统、Web 浏览器、Web 应用程序等等（可以存在于体系结构的所有层级中），但通常在最靠近前端的层级中使用。它类似于短期记忆且速度快。注意多级缓存系统可能会带来很多系统复杂性与变数，要视业务场景（数据一致性）判断使用，其中云计算时代使得本地缓存可能实际上没有节省内存资源成本（反而可能增加），从性能上来看更多地是优化了网络 IO 部分（避免 IO 延时以及节省了网络带宽），网络带宽是和 CPU、内存同等重要的资源、容易被低估，因为单独服务器带宽超限后会表现为隐性的网络变慢和超时，类似自我 DoS 然后或使得用户请求不可用或因健康检查异常被间接杀死。  
![](./cache-systems.jpeg)  
