...
## MongoDB
它的名字来源于 humongous 一词，意为 “巨大的”，体现了其设计目标 - 处理海量数据。MongoDB 是一个高性能、开源、无模式的 NoSQL 数据库，属于文档型数据库。[Ref](https://www.youtube.com/watch?v=-bt_y4Loofg)  
MongoDB 设计之初是为了填补同时期关系型数据库在可扩展性和灵活性方面的不足。但是随着关系型数据库（及其他 NoSQL）的发展（比如学习 MongoDB 引入了 JSON/JSONB 数据类型支持 schema-less，以及对分布式越来越友好的支持），MongoDB 的优势逐渐减弱，使用场景越来越少。  
  
* 对数据的操作大部分都在内存中，但 MongoDB 并不是单纯的内存数据库（事实来源还是存在磁盘）。
* MongoDB 是由 C++ 语言进行编写的，是一个基于分布式文件存储的开源数据库系统。
* 在高负载的情况下，添加更多的节点，可以保证服务器的性能。MongoDB 旨为 WEB 应用提供可扩展的高性能数据存储解决方案。
...
### MongoDB 的存储特点
在旧的、传统的关系型数据库中，数据是以表单为媒介进行存储的，每个表单均拥有纵向的列和横向的列。相比较 MySQL，MongoDB 以一种直观文档的方式来完成数据的存储。它很像 JavaScript 中定义的 JSON 格式，不过数据在存储的时候 MongoDB 数据库为文档增加了序列化操作，最终存进磁盘的其实是叫做 BSON 的格式，即 Binary-JSON。  
...
关于文档型数据类型，即可以存放 xml、json、bson 类型系的数据。这些数据具备自述性（self-describing），呈现分层的树状数据结构。数据结构由键值（key => value）对组成。  
  
### JSON/JSONB 数据类型的适用场景
数据不是特别重要（例如通知，推送这些），数据表结构变化较为频繁，数据量特别大，数据的并发性特别高，数据结构比较特别（例如地图的位置坐标），又比如数据库具体数据格式不明确或者数据库数据格式经常变化的需求模型，自带了对 Map-Reduce 运算框架的支持（很方便进行数据的统计）。从 data models 设计阶段就将原子性考虑于其中，无需事务之类的辅助。  
  
### MongoDB 缺点
* 只有 schema-less（本身是一把双刃剑）没有 schema support（也就不能做 JOIN 等复杂操作），不像现在关系数据库两种都支持了，分布式扩展能力也没用特别优势、不如 ScyllaDB 等等，而且还有许多大大小小的技术缺陷、问题。
* MongoDB 占用空间过大，因为它属于典型空间换时间原则的类型。那么它的磁盘空间比普通数据库会浪费一些。
* 关于事务，MongoDB 从 4.0 版本开始已支持多文档事务，但是只在单个分片上高效、且跨分片事务性能较差，建议尽量避免。
* [Ref 1](https://www.reddit.com/r/node/comments/i0l97y/why_does_mongodb_get_so_much_hate_genuine_question/?tl=zh-hans)、[Ref 2](https://www.zhihu.com/question/360088581)、[Ref 3（带技术细节）](https://www.youtube.com/watch?v=YdhTXxdTYso)
...
# 图数据库
用于解决复杂的多对多关系（复杂关联）或深度关系等问题（当关系数据库无法胜任其复杂性、性能要求或数据量扩展性时，同时值得留意的是图数据库支持 ACID 事务）。虽然 PostgreSQL 已经非常强大（支持递归查询、JSONB 等），但它的核心逻辑依然是 “基于表的连接”，而图数据库的核心逻辑是 “基于指针的遍历”。以下是图数据库在三个核心维度上的碾压级优势：
1. 查询性能：比如处理多层关联时，关系数据库递归查询每一层关联都需要进行一次 JOIN 即要两个表索引对比然后还要回表随机读取磁盘，图数据库使用的是无索引邻接（Index-Free Adjacency），每个节点直接存储了指向其邻居节点的物理内存地址，查询层层关联就像是沿着物理导线直接走过去，不需要做索引比对，在数学、复杂度上验证 - 假设集合为 N，查询 K 层关系的数据，且每一层出度平均为 M 时，那么关系数据库的查询总复杂度大概为 $O\left( \sum_{i=0}^{K-1} M^i \cdot \log N \right)$，图数据库查询复杂度大概为 $O\left( \sum_{i=0}^{K-1} M^i \right)$，这里虽然好像关系数据库只多了个 logN 乘数但其实际代表索引对比 - 即一个常数的物理工程耗时，那么当 $\sum_{i=0}^{K-1} M^i$ 较大时这个常数耗时也会被乘法放大。结果就是在处理 10 层以上的深度关系时，图数据库可能只需几毫秒，而 SQL 可能要很久甚至跑不出来。
2. 灵活的 Schema-less 线关系建模：关系数据库如果想增加一种新的关系（比如用户和商品之间增加一个收藏关系），必须更新、定义相关表结构 - 改表结构、加外键、考虑约束。图数据库数据模型就是白板上的点和线，可以随时增加一种新的 Label（标签）及其 Relationship（线），不需要停机维护，非常适合业务逻辑频繁变动的场景（如反欺诈引擎）。
3. 语义表达 Cypher vs SQL：深度关系查询时 SQL 需要用到复杂的 WITH RECURSIVE 递归查询，写出来可能要几十行，且极难调试。Cypher 只需要一行：`MATCH (:XXX {id: 'xxx'})-[:RELATED*1..3]->(related) RETURN related` 语法极其直观，几乎就是把白板上的连线图直接写成了代码。
4. 因为其底层实现，天然地更好地支持图数据结构算法：发现环、最短路径、中心性分析、知识图谱等。

图数据库内置了许多图算法，以提供对图数据的高级分析功能。  
常见应用场景：
