...
### 什么是持久化执行系统
持久化执行系统（Durable Execution System）确保应用程序中的代码即使面对各种故障和异常，也能可靠且正确地运行。它会维护应用程序状态，使代码能够自动从故障中恢复。无论故障是由网络超时这样的小问题引起的，还是由生产环境应用服务器发生内核崩溃（kernel panic）这样的大问题引起的，系统都能帮助应用程序继续执行。  

#### 提升开发者生产力
持久化执行系统还能提升开发者的生产力。作为开发者，知道必须处理故障、超时等会影响应用程序可靠性的问题，因此往往需要投入大量时间编写相关代码。  

Temporal 为应用程序开发提供了更高层次的抽象和内置的可扩展能力，让开发者可以将精力更多地放在应用程序的业务逻辑上。此外，Temporal 还提供了能够提升开发效率的工具，例如 Web UI。通过 Web UI，可以查看应用程序当前和过去的执行历史，包括输入参数和返回值。  
下面将使用 Web UI 调试 Workflow Execution，然后确认修复方案是否真正解决了问题。  
