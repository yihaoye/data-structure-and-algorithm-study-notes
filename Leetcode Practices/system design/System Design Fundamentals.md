...
* ![](./api-protocols.jpeg)
  ...
  * GraphQL，主要解决了一些传统 API 的问题，包括过度获取数据、多个端点、版本控制和文档理解等。在实现上，GraphQL 服务端需要定义 Schema、实现解析器、处理查询请求等步骤，以提供客户端所需的数据查询和操作功能。简而言之其通过优化客户端与服务端之间的数据传输冗余（也适用于两个服务端之间的调用），提高了网络 IO 的效率和性能。
* ![](./api-performance-improve.jpeg)
