...
因为要模仿散列或数组 O(1) 效率，因此可以使用 KV 数据库如 Redis 来存储分层时序槽，并且在数据量大的时候进行一致性哈希均匀分布。每个 Redis 实例存储一个完整的分层时序轮（但是每个实例存储的 Task 是唯一的），如此可以再为每 N 个实例分配一个 Worker 在每个最小时间段内检索时序轮的 ready Tasks 并将其放入消息队列中（同时给 Redis 更新这些 Task 的下一个运行时间）。  
