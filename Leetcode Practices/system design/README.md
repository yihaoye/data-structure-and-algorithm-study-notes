...
在进行导航时，因为已经由客户输入出发与目标地址，因此简单的设计是可以直接使用 `BFS`（不考虑权重）或 `Dijkstra`（考虑权重）算法（因为地图里每个点可以用二维图数据渲染）且预处理计算过的路径数据（该数据可以持久化存储，因为地图道路通常较少变更），另外对于快速判断两个地点是否有路径相连还应该引进并查集（为防止集合太大且没有连接时的大复杂度计算）。  
