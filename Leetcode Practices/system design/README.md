...
[前置阅读：架构、复杂度与三原则](../../Quality%20Assurance%20and%20Best%20Practice/架构、复杂度与三原则.md)  
而且[系统设计与现实生活有许多共通的哲学与智慧](https://www.youtube.com/watch?v=th_73AVA4dY)，例如二八法则、边际效益递减、系统冗余、机会成本（取舍策略）、墨菲定律、正负反馈（马太效应）、连锁反应（耦合解耦）、过滤分工分级（类似[分级诊疗](https://zh.wikipedia.org/wiki/%E5%88%9D%E7%B4%9A%E7%85%A7%E8%AD%B7)）、长尾效应、飞轮理论等等。  
...
* [断路器模式](https://docs.microsoft.com/zh-cn/azure/architecture/patterns/circuit-breaker) - 连接到远程服务或资源时处理故障，此类故障所需修复时间不定，这可以提高应用程序的稳定性和复原能力。但在处理对应用程序中的本地私有资源的访问，例如内存中数据结构，不推荐使用断路器模式，因为在此环境中使用断路器会增加系统开销。
  ...
  * 断路器模式的目的与重试模式（退避策略）不同。重试模式在预期操作将成功的情况下让应用程序重试操作。断路器模式则防止应用程序执行很可能失败的操作。应用程序可以使用重试模式通过断路器调用操作，来组合这两种模式。重试逻辑应该在断路器指示故障为非临时性的情况下放弃重试尝试，针对可能失败的操作，应监视最近发生的失败次数，并使用此信息来决定是允许操作继续进行，还是立即返回异常。
    * 退避重试的目的也是给下游减压，通过等待减少请求频率，让下游有喘息空间，但调用方自己的资源（比如线程池、连接池等）在退避等待期间如果被占用，并发请求一多则会资源耗尽或超时、报错堆积造成级联雪崩，在这种场景下断路器更适合，因其最核心的价值就是防止级联雪崩。
...
* 对象/文件/非结构化数据存储（S3、Hadoop HDFS、Blob Storage - Binary Large Object Storage）（e.g. distributed file storage system for storing photos and videos，另外其实数据湖 i.e. Data Lake 本质上也是 blob storage 且存的是 CSV 这类半结构化文件或者说 raw data）
...
![](./10-system-design-trade-offs-you-cannot-ignore.png)
...
在分布式系统中，心跳其实就是从一个组件发送到另一个组件的定期消息，用于交换数据或者健康状态，心跳可以是单向的，也可以是双向的。[Ref](https://juejin.cn/post/7383982728734392357)
* 单向心跳包 - 由一个节点（发送者）定期向另一个节点（接收者）发送消息，而不要求接收者回复。这种方式简单而高效，适用于一些基本的健康检查和状态监控。
* 双向心跳包 - 涉及两个节点之间的双向通信。一个节点发送心跳包，接收者收到后回复确认消息。这种方式能够提供更可靠的状态信息。

为什么需要心跳机制？因为分布式系统可能包含很多服务器，形成错综复杂的网络交互，产生的问题也是形形色色，假如没有心跳机制，可能会出现以下问题：
* 无法及时感知故障检测
* 服务器停机时间和错误会增加
* 整体分布式系统的可靠性会降低
...
注意，一般的消息队列（Kafka、Redis、ActiveMQ etc）不支持索引查询，但是一些关系型数据库（PostgreSQL）、时序数据库（时间序列数据库 Time Series Database，如 InfluxDB、MongoDB、Prometheus、RedisTimeSeries etc）除了能当简单的消息队列（比一般数据库吞吐性能更强，但仅限低吞吐量等有限场景。大规模、高吞吐量场景还是要用专门的消息队列系统）还可以索引查询（时间序列数据库通常会使用时间戳作为主要的索引字段，以便快速按时间范围查询数据。这使得在时间序列数据库中执行时间范围查询非常高效），而可以使用数据库来替代的重要原因也是因为新特性 [SKIP LOCKED](https://www.modb.pro/db/69904) 带来的性能以及可能性（具体比如 `SELECT ... FOR UPDATE SKIP LOCKED` 或 `SELECT ... FOR SHARE SKIP LOCKED`，因为传统的 `SELECT ... FOR UPDATE` 锁定会导致其他并发请求阻塞因为它们也查询命中了相关被锁行、另外还有死锁的风险，[更多参考](../../Computer%20System%20Layer/数据库/SQL/README.md#显式锁语法)）。另外，[有些消息队列可以直接支持优先级排序]((./消息队列与流处理.md#优先队列))而不是仅仅 FIFO。  
...
* 异步处理
  * 回调模式 - 系统级：Webhook、API 调用等；线程/进程级：JavaScript 回调、CompletableFuture、接口回调等。通常情况下，回调是在一个不同于主线程/主服务的另一个线程/服务中执行的。回调常用于异步处理中，当某个特定事件发生时，会触发回调函数的执行。[Ref](https://www.51cto.com/article/645183.html)
...
Stateful 避免了向不相关实例广播事件（或减少了实例需要过滤掉不相关数据比如全局数据库数据集）的开销、并使得同服务器/实例内存内数据交互成为可能，因此更高效、更快。  
...
* 复杂性增加：维护和理解一个深层级联的业务逻辑变得非常复杂，开发人员需要理解更多服务的内部运作
* 数据一致性难题：跨越多个服务的分布式事务（如 Saga）本身就复杂，深度增加会使得补偿逻辑和故障恢复更加困难
* 开发协作麻烦：比如透传字段要改多个应用，需求迭代效率低

最佳实践建议 [Ref 1](https://blog.csdn.net/u012534547/article/details/142263528)：
* 尽量扁平化调用链，控制直接调用深度：尽量将核心业务流程的直接串联调用控制在较浅的层级（例如，2 到 3 层是一个常见的经验法则）
  * 服务合并、重构 - 合理划分服务边界，高内聚，低耦合
    * 避免贫血领域模型：不要将所有数据操作暴露为简单的 CRUD API。让服务拥有自己的业务逻辑，减少需要多服务协作才能完成的跨服务事务
    * 避免过度拆分：不要为了拆分而拆分。如果两个功能总是同时变化、同时部署且高度依赖，它们可能应该属于同一个服务
  * 本地化处理 - 采用库而非独立服务
* 合理采用异步架构 - 事件驱动架构 (EDA)
* 集成强大的可观测性工具，链路追踪与优化
...
|-- |-- |--
|方案量级 | 轻量级 | 重量级
|运行环境	| 在客户端应用的进程内运行 | 在独立的服务器进程中运行
|性能/延迟	| 极低延迟 (内存中的函数调用)	| 较高延迟 (涉及网络往返)
|耦合度/依赖	| 紧耦合 (版本强依赖，部署更新库必须随主应用一起发布新版本)	| 松耦合 (只要 API 契约/协议不变，可独立更新、回滚)
|技术栈异构	| 语言特定 (通常需要为不同语言开发不同版本的库)	| 语言无关 (任何语言的应用都可以通过 API 调用)
|数据与状态	| 通常是通用的辅助的逻辑、无状态的工具类代码，或状态由主应用管理 | 有状态，自己管理自己的数据和数据库
|可靠性/故障 | 错误逻辑或 Bug 比如内存泄漏会殃及主应用 | 架构上提供了隔离和处理故障的可能性
|运维与基础设施成本	| 几乎为零 | 高 (需要额外的服务器、设备及网络资源、部署流水线、监控、日志等)
|安全性	| 业务逻辑和密钥暴露在客户端，易被逆向工程 | 核心逻辑和数据保留在安全的服务器端
...
另外可以适当讲讲如何在这里采用[图数据库](./NoSQL.md#图数据库)以及其优势、原理。  

[News Feed](https://www.woshipm.com/pd/2221252.html)
* EdgeRank 算法 - 是 Facebook 早期用于个性化排序用户动态消息（News Feed）的核心算法，帮助 Facebook 从信息爆炸中筛选出用户想看的内容，提升了用户粘性。虽然 EdgeRank 奠定了基础，但现代的 Facebook（Meta）算法已经发展得更为复杂和精细，引入了机器学习、更多信号（如观看视频的时长、点击率）来预测用户兴趣，但其核心思想（个性化、相关性排序）依然存在。主要有三个因素在起作用：
  * 亲密度：量化考虑平常里和这个朋友互动是否频繁，连接是否紧密。是点赞之交，还是评论之交，或是私信之交。
...
* （不可行方案）传统 SQL 直接查询
  ...
  * 假设先 `BETWEEN X-D AND X+D` 且 `BETWEEN Y-D AND Y+D` 然后再计算半径条件呢？是否能先过滤掉大部分数据达到优化？结论是不能，下面分析说明无论是分别进行索引还是联合索引，时间复杂度都仍不理想。
    * 首先基本原理与 2D 数组排序类似，无论怎么样都只能先对其中一个字段进行全局排序，第二个字段进行局部排序（第一字段同值下第二字段顺序）。因此分别索引的情况下，数据库只能索引优化范围查询其中一个字段，而第二个字段只能全量暴力过滤，假设最终满足 lat 的数据集为 H，总数据量 N 个，那么最终时间复杂度为 `O(logN + H)`，注意这里数据库只会实际使用其中一个字段的索引不会两个都用，因为如果把两个都先分别索引优化范围查询然后再计算交集，则时间复杂度更差空间复杂度占用也更多；复合索引的情况下，理论上时间复杂度应该是 `O(M * (logN + logW + K/M))`，因为第二字段是局部有序的，所以每次到下一个 lat（第一字段）的值要回溯一下不能发挥叶子节点链表的作用，假设总 lat 数量为 N，最终满足 lat+lng 的结果集应为 K，假设满足条件查出的 lat 独立值（比如 39.000001，这里每个独立值可能对应多个 lng - 简化假设平均是 W 个）有 M 个，那么这里时间复杂度的意思是每次 lat 独立值先定位消耗 logN * M（因为每次都要重新走树而不是链表），然后每次在里面进行局部二分搜索范围查询 K/M 个 lng - 即消耗 log(W) 先定位到第一个符合条件的 lng，然后走链表查完后面的 K/M 个，如此类推，虽然看起来比分别索引好，但实际上因为回溯是随机 IO 不如链表 IO 效率，数据库无法判断哪个更好，所以实际的查询优化器仍然会将其退化为第一种情况即一维索引+剩下暴力过滤 `O(logN + H)`。[这个联合索引失效的问题与通常企业里要求的最右侧或尽量右侧范围查询的联合索引优化规范准则是同理的。](../../Computer%20System%20Layer/数据库/SQL/README.md#索引的优化与如何避免索引失效)
...
于是 Uber 后来不再使用 S2 方案，自研出 H3（Hexagonal Hierarchical Spatial Index）：前面的球面投影使用正方体，但通常六边形更好，且六边形个数越多越好，粒度越细，就越贴近球体（六边形的菱角比较少，六个边也能相互衔接其他的六边形，六边形足够多以后，非常近似球体）。Uber 用的是六边形的网格，把城市划分为很多六边形。  
...
PvP 游戏也是一个典型的 stateful 系统（并不是说所有 PvP 游戏都是如此，但是通常 PvP 如 MOBA、FPS、Sports、Fighting、RTS 都是低延迟、高并发、同时在线人数较多的情况下 stateful 更高效，如果是异步对战类游戏则要求较低也可以采用 stateless 设计），客户端与服务端之间需要保持实时的连接。一个是否需要 stateful 设计的推荐判断是玩家之间是否需要共享一个持续变化的运行时状态（shared runtime state）。  
...
* 低延迟：Pong 游戏体验要求极低的网络延迟比如 < 100ms RTT（Round-Trip Time），且多区域时要就近分配
* 高可用性：系统需要保证在高负载下仍能稳定运行

Pong 的特殊性 - 一场对局的全部状态就是两个球拍坐标 + 球的位置、速度 + 比分，几十个字节；固定 2 人对战，且对战时间短（十几秒到几分钟）。  

**基础架构设计**  
Login 登录部分和大部分互联网应用类似，使用 stateless 设计，标准 OAuth/用户名密码换 JWT，客户端携带 JWT token 访问游戏服务及保证所有业务请求正常调用所需的微服务接口，不需要回源查 session。  
同时登录成功后，接入网关（Gateway）会把玩家的在线状态写入 Redis，Redis 的 key 是玩家的 ID，value 是玩家的接入网关地址（即玩家所在的游戏服务节点）作为路由表（需要路由的原因是接入网关是集群，用户将与其建立长链接，不同的用户可能由不同的接入网关实例处理），如果玩家下线则接入网关会检测到并主动删除该 key，并设置 TTL（24 小时）这是因为如果网关实例宕机则无法主动删除，有了 TTL 后 Redis 最晚 1 天后会自动删除该 key，从而保证不会占用资源。  

当用户在客户端主动提交开始游戏时，系统才会进入下一步 - 匹配一对对战玩家。  
Matchmaker 匹配器是一个后台运行 worker 负责撮合玩家。如果根据玩家的 rating 找一个最接近的人，可以使用 Redis Sorted Set 来存储玩家的 rating，这个队列只有玩家提交开始游戏时才会将玩家 ZADD 进去，Sorted Set 的 score 就是玩家的 rating（具体数字可以是玩家过往的战绩，记得在此之前从关系数据库里获取），value 是玩家的 UID。一个 matchmaker 进程轮询配对，匹配时可以使用 ZRANGEBYSCORE 命令来获取指定范围内的玩家 UID，然后从中随机选择一个玩家进行匹配（即使 100 万 CCU、每秒几千次入队，单个 matchmaker 进程也绰绰有余，因为内存配对是微秒级。实际情况下，用排队论的 Little's Law 估算：L = λ × W 即队列长度 = 到达率 x 平均逗留时间，假设玩家每 10 分钟发起一次匹配，到达率则为 ~= 1667 人/秒，平均等待 5 秒的话，则队列稳态长度约为 8000 人，所以还是很少数据）。  
Matchmaker 可以设置为每 100ms 批量处理一次匹配请求，尽量一次性全部配完（有可能有落单因此下一轮继续处理）。匹配算法有很多种，最简单的是下面贪心算法：
* [ELO 评分](https://zh.wikipedia.org/wiki/%E7%AD%89%E7%BA%A7%E5%88%86)加上扩张搜索窗口 — 玩家进入队列后，先在 ±20 ELO 范围内搜索，每隔几秒扩大范围，平衡匹配质量和等待时间。如果要更精准，可以用 [TrueSkill](https://zh.wikipedia.org/wiki/TrueSkill%E8%AF%84%E5%88%86%E7%B3%BB%E7%BB%9F) 替代 ELO，因为它有不确定性参数，新玩家收敛更快。

接下来 Allocate 分配游戏是该系统核心困难分布式工程（Allocator 和 Matchmaker 可以是同一个进程的两个模块，在 Matchmaker 每匹配完一对后调用一次 Allocator；也可以根据具体业务分为 2 个服务）。撮合成功后把两人放进同一个对局进程 - 即选择游戏权威服务器节点（权威服务器 Authoritative Server 的意思是服务端运行游戏逻辑，客户端只负责渲染和输入，服务端才是游戏的最终裁决者，反之则是客户端权威即客户端跑逻辑、服务端只负责转发给其他客户端，这里选择权威服务器的原因是：1. 防止作弊；2. 更好保障一致性）。权威服务器通常是 stateful 的，因为它需要维护游戏状态，并且需要与客户端保持实时连接且最好是长链接。Allocate 关键点如下：
* 容量视图：每台对局服务器定期心跳上报 “还能开几个房间” 到 Redis 并带上 TTL，Allocator 从中挑一台。心跳 + TTL 可保证机器挂了会自动从 Redis 分配池消失。
* 分配确认：Allocator 选中节点后先向该节点（权威服务器进程的 RPC 调用）预留房间（该进程维护比如 1000 个房间，key 为房间 ID 值记录 match_id 以保证幂等性和对战双方 uid）防止并发注册超资源以及供后续业务使用，成功后再将服务器地址及 match_id + 对战双方 uid + 用户对应的接入网关到 redis 的 pub/sub，接入网关之前已经与 Redis 建立了连接，并订阅了与其网关相关的频道，检测到分配完成后就会通过长链接推送结果（权威服务器进程地址 ip:port）给客户端。

开始对局，客户端发起另一个长链接请求到分配的权威服务器节点，权威服务器节点收到请求后验证双方 uid（解析 JWT 获取）是否匹配成功，成功后开始游戏，身份等静态信息只有第一次连接时需要解析，之后都在长链接会话里或者比如 Go 程序可以每个长链接一个协程长持处理，协程的 context 里有 JWT 或者函数入参传递即可。  
若失败或掉线，系统仍会留有一定的超时重试机制，客户端可以重新发起连接。如果超时后仍旧无法连接（此时权威服务器内部可以有后台线程检测用户状态把非正常进行游戏的房间与对手释放掉 - 建议对掉线选手判负以避免玩家作弊总是在快输之前拔网线刷分；游戏一直未开始，则把到场者保留原匹配入队时间回队，缺席者不回，否则可能会出现幽灵用户，因为用户可能是彻底断网或关机不再上线；也可能根本原因是权威服务器实例宕机 - 此局作废不判输赢）。  
所有的对战信息状态（比如球的位置、速度、双方球拍位置等）都在权威服务器节点内存里维护，并定期把游戏状态推送给双方客户端。  
注意游戏服务实例无论宕机重启或按需扩展收缩，都有一个程序少不了 - 服务发现、注册，权威服务器节点启动成功后向 Redis 注册自己及可用房间等等（并在之后以定期更新），Allocator 也从 Redis 获取可用的权威服务器节点列表（列表本身也可以是优先队列）。  

![](./pong_backend_final_architecture.svg)  
![](./full_match_lifecycle_flow.svg)  

**存储设计**  
Redis：
* 在线用户状态表：key 为 uid，value 为接入网关 ID，TTL 过期时间为 24 小时
* Matchmaker 排队表：Sorted Set，score 为玩家 rating，value 为玩家 uid
* 权威服务器节点列表
* 权威服务器节点的可开房间数，TTL 过期时间为 30 秒
* 地区或者全球的排行榜，Sorted Set，score 为玩家 rating，value 为玩家 uid

关系数据库比如 PostgreSQL：
* 用户表：存储用户的基本信息、战绩等
* 对局表：存储每场对局的结果、双方 uid、分数等信息

**API 设计**  
* Matchmaking API
  * WS C->S - 开始匹配，客户端发送请求后，Matchmaker 将玩家加入排队表
  * WS C->S - 取消匹配，客户端发送请求后，Matchmaker 将玩家从排队表中移除
* Game Allocation API
  * POST /game/allocate - 分配游戏，Matchmaker 调用 Allocator，返回权威服务器地址和 match_id
* Game State API
  * WS C->S - 获取游戏状态，客户端向权威服务器请求当前游戏
  * WS C->S - 更新用户动作（上/下/停，频率）
  * POST /game/score - 提交分数，权威服务器向计分服务发送最终分数，计分服务更新数据库和排行榜
  * GET /game/leaderboard - 获取排行榜，客户端向计分服务请求排行榜数据

**进阶**  
* 多区域部署：根据玩家的地理位置，将接入网关、Matchmaker/Allocator 和权威服务器部署在不同的区域，以减少网络延迟，这样只有同一地区的玩家才会被匹配到一起，保证游戏体验。这里需要取舍考虑成本，比如是否细分到大区还是小区，但是因为 Pong 游戏对延迟要求极高，建议尽量细分到小区（比如城市级别）或者至少不跨海底电缆，以保证玩家之间的网络延迟尽可能低。
  * 全球数据需要同步，比如排行榜、玩家战绩等，可以使用异步数据同步机制，将各区域的数据汇总到一个中心数据库，或者使用分布式数据库来实现数据的同步和一致性。
* Redis 使用集群（有主从备份），防止单点崩溃。
...
**系统要求与目标**  
* 功能性与非功能性需求
  * 能够极快地（到微秒级，即百万分之一秒）获取并处理大量的传入信息 - 对外部事件的响应时间，内部响应时间，以及提供最高吞吐量和最低延迟的能力。
  * 在超高速的同时还要严格保证数据、操作、逻辑的准确性。
...
**语言选择与内存管理**  
在高频交易中，延迟的确定性（Predictable Latency）和避免抖动（Jitter）比单纯的低延迟更为重要。任何不可预测的暂停都可能导致错失交易机会或做出错误决策，P99（尾延迟）在 HFT 中极其致命。  
* GC 的 STW 问题 - 像 Java、Go 等带有自动垃圾回收（GC）的语言，其 GC 过程通常会引发 Stop-The-World（STW）暂停。尽管现代 GC 暂停时间很短（几毫秒），但这在以微秒（μs）甚至纳秒（ns）计的 HFT 领域是致命的。一次 STW 暂停意味着成千上万个交易机会的错失。
* C++/Rust 的优势
  * C++ - 是 HFT 领域的传统霸主。它允许开发者进行完全手动的内存管理。通过在启动时使用内存池（Memory Pools）等技术预分配内存，可以避免在交易时段进行任何动态内存分配，同时避免 GC 带来的不确定性。
...
* 物理位置 - 将交易服务器放置在交易所附近（Co-location）甚至直接租用交易所数据中心机房内的机柜以最小化信号传播延迟，是 HFT 的常见做法。
...
### 证券交易所系统
https://bytebytego.com/guides/design-stock-exchange/  
![](./Stock%20Exchange%20System%20Design.png)  
步骤 3：订单通过客户端网关进入交易所。客户端网关执行基本的网关功能，例如输入验证、速率限制、身份验证、规范化等。然后，客户端网关将订单转发给订单管理器。  
步骤 4 - 5：订单管理员根据风险管理员设定的规则执行风险检查。  
步骤 6：通过风险检查后，订单管理员会核实钱包中是否有足够的资金来支付订单。  
步骤 7-9：订单被发送至匹配引擎。当匹配成功时，匹配引擎会发出两个执行指令，分别对应买方和卖方。为了保证重放匹配结果的确定性，订单和执行指令均按顺序执行。  
步骤 10 - 14：将执行结果返回给客户端。  
请注意，交易流程（步骤 1 至 14）位于关键路径上，而市场数据流程和报告流程则不在关键路径上。它们的延迟要求不同。  
...
**High Level Design**  
整个网络就像一张图，爬取的过程就如有向图的遍历过程，其中网页作为节点，超链接（URL）作为边。抓取过程可以被视为从一个网页到其他网页的有向图的遍历。两种常见的图形遍历算法是 DFS 和 BFS。然而，DFS 通常不是一个好的选择，因为 DFS 的深度可能很深。  
BFS 通常被网络爬虫使用，并通过先进先出 (FIFO) 队列实现。在 FIFO 队列中，URL 按照它们入队的顺序出队。但是，这种实现有 2 个问题：
* 标准的 BFS 没有考虑到一个 URL 的优先级。网络很大，不是每个页面都有相同的质量和重要性。因此，需要根据页面排名、网络流量、更新频率等来确定 URL 的优先级。
* 来自同一网页的大多数链接都链接回同一主机。比如 wikipedia.com 中的所有链接都是内部链接，使得大部分甚至所有爬虫都忙于处理来自同一主机（wikipedia.com）的 URL。当爬虫试图并行下载网页时，维基百科服务器将被请求淹没。这被认为是不礼貌的。![](./wikipedia-web-tree.avif)
  
**因此整个设计也有点像算法题实现图遍历 - 并且是带优先队列以及去重的有向图遍历**。而首先需要给定一些起始爬取的点（seed URLs），又因为互联网有海量网站，因此爬取需要有优先级以提高效率 / ROI  
然后执行以下步骤：  
...
### 设计词云系统
Word Cloud，属于 Top K 系统的 follow up 更为复杂，因为可以进行条件筛选比如目标对象、地区、语言、时间段进行聚合统计，在这种情况下可能会发生组合爆炸，使用 Count-min Sketch 表格的数量会过多（因为每个都是单维度的统计），不利于维护使用、设计或扩展，因此只能转向基于通用数据库来实现。  
可以使用列式数据库来存储（[列式数据也是为此而生的](./NoSQL.md#列数据库)），其中每行数据的设计如下：  
...
词云系统也可以由 ES 全文搜索实现（分词聚合），其中有 2 种办法（但是多数情况下只有第 1 种可行）：
* doc_values：推荐，默认开启的，使用的是磁盘（不会 OOM），实际本质也是以上面的列式存储实现。但是注意它应该是针对非 text 字段（默认对 text 也是关闭的），所以文本内容如果为 text 字段是无法复用的，需要另一个 nested 类型的字段（如下所示，或者 keyword 数组字段），然后在写入数据（比如上传原文）的同时仍然需要在应用层先进行分词（比如使用第三方库 Jieba 或者调用 ES 分词 API 给定文本和 analyzer 获取返回的分词结果、又或者使用 ES Ingest Pipeline）然后写入到该字段（没错，ES 无法自动在对 text 字段分词时顺便写入到该字段）所以本质上与列式数据库方案基本类似，以便高效地进行词云聚合。[Ref 1](https://www.cnblogs.com/juniorMa/p/16198364.html)、[Ref 2](https://cloud.tencent.com/developer/article/2398533)、[Ref 3](https://cloud.tencent.com/developer/article/2228842)、[Ref 4](https://www.cnblogs.com/leeSmall/p/9215909.html)、[Ref 5](https://zhuanlan.zhihu.com/p/107820698)
```json
"words": {
  "type": "nested",
  "properties": {
...
```  
* fielddata：不推荐，默认是关闭的需要自己开启，使用的是内存。不过它的好处是比 doc_values 方便，因为直接针对 text 字段，所以设置为 true 后，就等于可以直接对 text 字段的分词结果执行聚合等查询（不需要像 doc_values 方案那样再加新字段并在应用层额外分词）。实际上数据量稍微较多的时候就会使得 ES 实际不可用（会 OOM，性能成本极高），原因接下来一步步分析这种聚合过程（实际情况是分布式、增量式的算法在分片级别并行执行，但仍然可见其成本）：
...
**如果实际词云数据中等偏小的话，可以（冗余）分表，比如虽然基础数据集较大，但是实际用于词云系统的数据记录较小时，也可以把这部分数据单独拿出来部署到独立的 ES 集群，这样可以直接开启使用 ES 内置功能 fielddata，避免从零实现词云功能（基于纯列式数据库的方案，虽然更麻烦些，但是更适用于海量数据、高性能要求 - 聚合性能 ClickHouse > ES）。但问题是从业务应用上来说，词云系统本身只有当数据量较大的情况下才有意义，所以该方案适合的应用场景非常有限而且极易崩溃（OOM 或 GC 压力）：比如峰值 QPS 10 的并发请求下单次经过条件过滤查询命中使用内存 ≤ 125MB 即约少于 5 万文档（比如时间等条件筛选后，因此全量数据较大比如千万也没问题）以下的聚合查询（假设节点内存 16GB，而且手动设置 fielddata_cache 等合理上限来触发 LRU，防止聚合很快把堆内存占满）。**  
  
存储与运维成本对比：  
存储成本 ClickHouse 显著优势，运维成本 ClickHouse 略有优势，总体成本 ClickHouse 在大数据量场景下优势明显。但要 ES 的生态更成熟，相对 ClickHouse 而言团队学习成本更低。对于词云系统，如果数据量大（TB 级别），强烈推荐 ClickHouse。  

参考：
* https://blog.csdn.net/laoyang360/article/details/141406861
* https://cloud.tencent.com/developer/article/2198030
* https://www.cnblogs.com/wangzhuxing/p/9508784.html
* https://cloud.tencent.com/developer/article/2423113
...
**方案一 RDBMS（推荐）**：这里选择了 RDBMS，因为稍后需要 ACID 属性（事务），设计一个任务表后，并发获取时采用 SQL 语法 `SELECT ... FOR UPDATE SKIP LOCKED` 则可以实现高并发下的事务（即如果处理都是纯数据库操作的话，比下面的 Redis ZSet 方案简洁稳妥）。数据库被分片成足够数量的分片来分配负载和数据。以半同步的方式对每个分区使用主动被动/主从复制。一个 Slave/Follower 将同步跟随，而其他的将异步接收 Replication Stream。这样，可以确保至少有一个 Slave 保存最新数据，以防 Master 失败（如网络分区、服务器中断等故障因素），并且 Slave 将被提升为新的 Leader。  

**方案二 Redis**：使用 ZSet 放置每个 job id 及其 next process time 作为分数，每次从 ZSet 中获取出来进行处理之后再更新放回去（而且 ZSet 是并发安全的，所以多消费实例也不用担心），且一般来说只要数据建模得当，单个 ZSet 可以轻松应付百万数量的任务。但是因为一般具体业务数据不放在 Redis，所以 Redis 方案将不可避免的会引入分布式事务 - 因为任务处理时可能中间出错、失败导致丢掉任务，如果不允许 skip 每一次任务 - 即不能在任务成功完成前往 ZSet 写入下一次的任务的话可以再加一个处理中的 ZSet 并通过 Lua 脚本一取一写保证原子性（处理完后从第二个 ZSet 取出往第一个 ZSet 写入下一次任务，另外还有一个后台轮询第二个 ZSet 的失联任务重新放入第一个 ZSet，因此处理部分也需要幂等性设计保证多次处理也没有影响），之所以需要两个 ZSet 本质上是因为 Redis 没有类似 `SELECT ... FOR UPDATE SKIP LOCKED` 机制，而如果可以忍受一定的任务失败或 skip 的话则可以在每次读取任务后立刻写入下一次任务（任务重试仍失败如果需要记录的话可以写入死信队列），而如果担心内存数据库断电丢数据的话开启 AOF 即可。  

这里注意到上面 2 种方案都涉及了分布式事务：方案一如果可以把具体业务处理与读写任务的数据库分区放在同一个分区（即使数据库本身是分布式集群），就可以单机单事务完美完成；而方案二则除非相关业务数据都放在 Redis 的话才可以通过 Lua 脚本较完美完成（但 Redis 分布式集群本身也仅是最终一致性）。  
...
* https://bytebytego.com/guides/how-are-notifications-pushed-to-our-phones-or-pcs/
...
[具体代码逻辑及其实现](../../Other%20Practices/game-of-life/Solution.java)  
...
[存储过程](../../Computer%20System%20Layer/数据库/SQL/README.md#Stored-Procedure)架构即把部分或全部业务规则、逻辑直接写入数据库中，例如使用存储过程（Stored Procedure）、触发器（Trigger）、自定义函数（Function）和约束（Constraint）等特性。应用层只负责参数传递、UI 展示。[Ref](https://juejin.cn/post/7515597690945191936)  
这种架构适用于普通数据量、普通用户量且逻辑不需要跨库的系统（单机），优点在于简便且高性能、安全。缺点是难以测试、系统复杂较大的话难以维护甚至无法支持。因此通常不适合互联网系统。  
...
### 批量操作（Bulk Operation）
其实与缓冲思路类似，都是对 I/O 性能优化，区别是批量操作在 I/O 调用上控制而缓冲是 I/O 里的优化，无论是批量操作还是缓冲，它们的核心思想都是：将多次小的 I/O 操作合并成一次大的 I/O 操作。  
批量操作在工作中更实用、常见，是因为它发生在应用的业务逻辑层，而缓冲是发生在系统底层或传输层。  

定义：一次性、以最大效率对大量数据执行同一个操作（读取、写入、更新）。  
原子性：强调将 N 次独立操作合并为 1 次 I/O 操作，从而减少网络往返时间（RTT）、数据库事务开销和协议开销。  
规模性：通常用于描述一次性导入或导出大量数据。示例：在一个数据库中，Bulk Insert 指的是将 1000 条记录打包在一个 SQL 语句中提交，而不是执行 1000 条独立的 INSERT 语句。这是为了提高 I/O 效率。  
...
## 客户端离线数据同步
Ref:
* https://juejin.cn/post/7408008683366613043
