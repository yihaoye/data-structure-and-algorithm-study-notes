...
* doc_values：推荐，默认开启的，使用的是磁盘，实际本质也是以上面的列式存储实现。[Ref 1](https://www.cnblogs.com/juniorMa/p/16198364.html)、[Ref 2](https://cloud.tencent.com/developer/article/2398533)、[Ref 3](https://cloud.tencent.com/developer/article/2228842)
* fielddata：不推荐，默认是关闭的需要自己开启，使用的是内存。实际上数据量稍微较多的时候就会使得 ES 实际不可用（性能成本极高），原因接下来一步步分析这种聚合过程：
...
**如果实际词云数据较小的话，可以（冗余）分表，比如虽然基础数据集较大，但是实际用于词云系统的数据记录较小时，也可以把这部分数据单独拿出来部署到独立的 ES 集群，这样可以直接使用 ES 内置功能，避免从零实现以上功能。**  
