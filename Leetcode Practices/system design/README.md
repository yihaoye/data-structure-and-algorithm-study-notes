...
注意词云系统可以由 ES 全文搜索引擎内置地支持（分词聚合），其中有 2 种办法（但是多数情况下只有第 1 种可行）：
...
**如果实际词云数据中等偏小的话，可以（冗余）分表，比如虽然基础数据集较大，但是实际用于词云系统的数据记录较小时，也可以把这部分数据单独拿出来部署到独立的 ES 集群，这样可以直接开启使用 ES 内置功能 fielddata，避免从零实现词云功能（基于纯列式数据库的方案，虽然更麻烦些，但是更适用于海量数据、高性能要求）。但问题是从业务应用上来说，只有当数据量较大的情况下词云系统才有意义，所以也与该方案可行性有所矛盾。**  
