...
### 微服务级联风险
在微服务架构中，级联深度（或称调用链深度）是一个非常重要的考量点（应尽量避免的），它直接影响系统的性能、可靠性、可观测性和可维护性：
...
最佳实践建议
* 尽量扁平化调用链
  * 控制直接调用深度：尽量将核心业务流程的直接串联调用控制在较浅的层级（例如，2 到 3 层是一个常见的经验法则）
  * 聚合服务/BFF (Backend for Frontend)：对于复杂的客户端请求，可以使用聚合服务或 BFF 层来封装后端多个服务的调用，将多余的级联处理移到这一层。这样，客户端只需要调用一个服务，而这个聚合服务负责内部的扇出（fan-out）调用。
* 合理采用异步架构 - 事件驱动架构 (EDA)
* 合理划分服务边界，并结合强大的可观测性工具
  * 高内聚，低耦合
  * 避免贫血领域模型：不要将所有数据操作暴露为简单的 CRUD API。让服务拥有自己的业务逻辑，减少需要多服务协作才能完成的跨服务事务
  * 避免过度拆分：不要为了拆分而拆分。如果两个功能总是同时变化、同时部署且高度依赖，它们可能应该属于同一个服务
* 服务网格 (Service Mesh) - 如 Istio、Linkerd 可以在不修改服务代码的情况下，提供流量管理、熔断、重试、限流、可观测性等能力。它能帮助管理级联调用中的复杂性，提高系统的弹性和可靠性
