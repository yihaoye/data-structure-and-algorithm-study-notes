...
**方案一 Redis**：使用 ZSet 放置每个 job id 及其 next process time 作为分数，每次从 ZSet 中获取出来进行处理之后再更新放回去（而且 ZSet 是并发安全的，所以多消费实例也不用担心），且一般来说只要数据建模得当，单个 ZSet 可以轻松应付百万数量的任务。如果担心任务处理时间太长可能中间出错、失败导致丢掉任务（且不允许 skip 每一次任务 - 即不能在任务成功完成前往 ZSet 写入下一次的任务）的话可以在中间再加一层持久层比如消息队列，而如果担心内存数据库断电丢数据的话开启 AOF 即可。  
