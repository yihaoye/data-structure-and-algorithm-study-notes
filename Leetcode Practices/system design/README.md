...
同时登录成功后，接入网关（Gateway）会把玩家的在线状态写入 Redis，Redis 的 key 是玩家的 ID，value 是玩家的接入网关地址（即玩家所在的游戏服务节点）作为路由表（需要路由的原因是接入网关是集群，用户将与其建立长链接，不同的用户可能由不同的接入网关实例处理），并设置 TTL，如果玩家在一定时间内没有任何操作，Redis 会自动删除该 key，从而保证玩家离线后不会占用资源。  
...
Matchmaker 可以设置为每 100ms 批量处理一次匹配请求，尽量一次性全部配完（有可能有落单因此下一轮继续处理）。匹配算法有很多种，最简单的是下面 2 种贪心算法：
* 按序一次性遍历，每邻接的 2 个为一对，优点是高效简单，缺点是落单的容易一直配不上、且匹配的双方战绩差距可能较大
* 按用户等待时间优先处理，查询目标用户左右 2 个玩家的 rating，挑战绩更接近的玩家匹配，优点是匹配更优不会有长期落单的问题，缺点是更复杂
...
若失败或掉线，系统仍会留有一定的超时重试机制，客户端可以重新发起连接。如果超时后仍旧无法连接（此时权威服务器内部可以有后台线程检测用户状态把非正常进行游戏的房间与对手释放掉，也可能根本原因是权威服务器实例宕机），则双方客户端回到第一阶段，可以重新提交开始游戏才会重新走一遍 Matchmaker、Allocate 等（否则可能会出现幽灵用户，因为用户可能是彻底断网或关机）。  
所有的对战信息状态（比如球的位置、速度、双方球拍位置等）都在权威服务器节点内存里维护，并定期把游戏状态推送给双方客户端。  
注意游戏服务实例无论宕机重启或按需扩展收缩，都有一个程序少不了 - 服务发现、注册，权威服务器节点启动时向 Redis 注册自己及可用房间等等，Allocator 也从 Redis 获取可用的权威服务器节点列表（列表本身也可以是优先队列）。服务注册可以采用业界常见方案如 Spring Cloud 等或 Service Mesh。  
...
**存储设计**  
Redis：
* 在线用户状态表：key 为 uid，value 为接入网关 ID，TTL 过期时间为 30 秒
...
**API 设计**  
* Matchmaking API
  * POST /matchmaking/start - 开始匹配，客户端发送请求后，Matchmaker 将玩家加入排队表
  * POST /matchmaking/cancel - 取消匹配，客户端发送请求后，Matchmaker 将玩家从排队表中移除
* Game Allocation API
  * POST /game/allocate - 分配游戏，Matchmaker 调用 Allocator，返回权威服务器地址和 match_id
* Game State API
  * GET /game/state - 获取游戏状态，客户端向权威服务器请求当前游戏
  * POST /game/state - 更新游戏状态，客户端向权威服务器发送当前游戏状态（球拍位置、球位置等）
  * POST /game/score - 提交分数，客户端向权威服务器发送最终分数，权威服务器更新数据库和排行榜
  * GET /game/leaderboard - 获取排行榜，客户端向权威服务器请求排行榜数据
