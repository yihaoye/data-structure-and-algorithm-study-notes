...
设计Twitter搜索
Twitter是最大的社交网络服务之一，用户可以分享照片、新闻和基于文本的信息。在本章中，我们将设计一个可以存储和搜索用户推文的服务。类似的问题。推文搜索。难度等级。中等
1. 什么是推特搜索？
Twitter用户可以随时更新他们的状态。每条状态（称为tweet）由纯文本组成，我们的目标是设计一个可以搜索所有用户tweet的系统。
2. 系统的要求和目标
   - 让我们假设Twitter有15亿用户，每天有8亿活跃用户。
   - 平均而言，Twitter每天有4亿条推文。
   - 一条推文的平均大小为300字节。
   - 让我们假设每天会有5亿次搜索。
   - 搜索查询将由多个词与AND/OR组合而成。我们需要设计一个能够有效存储和查询推文的系统。
3. 容量估计和限制条件
存储容量。由于我们每天有4亿条新推文，每条推文平均为300字节，我们需要的总存储量将是。
400M * 300 => 120GB/day
每秒总存储量：
120GB / 24hours / 3600sec ~= 1.38MB/second
4. 系统API
我们可以有SOAP或REST API来提供我们服务的功能；下面是搜索API的定义。
search(api_dev_key, search_terms, maximum_results_to_return, sort, page_token)
参数。
api_dev_key（字符串）。一个注册账户的API开发者密钥。这将被用于，除其他外，根据他们分配的配额来节制用户。
search_terms（字符串）。一个包含搜索条件的字符串。
maximum_results_to_return（数字）。要返回的推文的数量。
sort（数字）。可选的排序模式。最新的优先（0-默认），最佳匹配（1），最喜欢（2）。 page_token（字符串）。这个标记将指定结果集中的一个页面，应该被返回。
返回：（JSON）
一个包含符合搜索查询的推文列表信息的JSON。每个结果条目可以有用户ID和姓名、推文文本、推文ID、创建时间、喜欢的数量等。
5. 高层设计
在高层次上，我们需要将所有的雕像存储在数据库中，同时建立一个索引，可以跟踪哪个词出现在哪条推文中。这个索引将帮助我们快速找到用户试图搜索的推文。
6. 详细的组件设计
   1. 存储。我们每天需要存储120GB的新数据。鉴于这个巨大的数据量，我们需要想出一个数据分区方案，将数据有效地分配到多个服务器上。如果我们对未来五年进行规划，我们将需要以下存储。
   120GB * 365天 * 5年 ~= 200TB
   如果我们在任何时候都不希望超过80%的容量，我们大约需要250TB的总存储量。让我们假设我们想为所有的推文保留一个额外的副本作为容错；那么，我们的总存储需求将是500TB。
   那么，我们的总存储需求将是500TB。如果我们假设一台现代服务器可以存储4TB的数据，我们将需要125台这样的服务器来保存未来5年的所有所需数据。
   让我们从一个简单的设计开始，我们将推文存储在一个MySQL数据库中。我们可以假设我们将推文存储在一个有两列的表中，TweetID和TweetText。让我们假设我们根据TweetID来划分我们的数据。如果我们的TweetID是全系统唯一的，我们可以定义一个哈希函数，将TweetID映射到我们可以存储该Tweet对象的存储服务器。
   我们怎样才能创建全系统唯一的TweetID？如果我们每天都有4亿条新推文，那么五年内我们可以期待多少推文对象？
   400M * 365天 * 5年 => 7300亿
   这意味着我们需要一个五字节的数字来唯一地识别TweetID。让我们假设我们有一个服务，可以在我们需要存储一个对象时产生一个唯一的TweetID（这里讨论的TweetID将与《设计Twitter》中讨论的TweetID相似）。我们可以将TweetID反馈给我们的哈希函数，以找到存储服务器并将我们的Tweet对象存储在那里。
   1. 索引。我们的索引应该是什么样子的？由于我们的tweet查询将由单词组成，让我们建立索引，告诉我们哪个单词出现在哪个tweet对象中。让我们首先估计一下我们的索引会有多大。如果我们想为所有的英语单词和一些著名的名词如人名、城市名等建立一个索引，如果我们假设我们有大约30万个英语单词和20万个名词，那么我们的索引里将有50万个单词。让我们假设一个词的平均长度是五个字符。如果我们在内存中保存索引，我们需要2.5MB的内存来存储所有的单词。
   500k * 5 => 2.5 MB
   让我们假设，我们想在内存中保存仅过去两年的所有推文的索引。由于我们将在5年内得到730B条推文，这将使我们在两年内得到292B条推文。鉴于每个TweetID将是5个字节，我们需要多少内存来存储所有的TweetID？
   292B * 5 => 1460 GB
   因此，我们的索引就像一个大的分布式哈希表，"键 "是单词，"值 "是包含该单词的所有推文的TweetID列表。假设每条推文中平均有40个词，由于我们不会对介词和其他小词如'the'、'an'、'and'等进行索引，让我们假设每条推文中大约有15个词需要被索引。这意味着每个TweetID将在我们的索引中被存储15次。因此，我们将需要总的内存来存储我们的索引。
   (1460 * 15) + 2.5mb ~= 21 tb
   假设一个高端服务器有144GB的内存，我们将需要152个这样的服务器来保存我们的索引。我们可以根据两个标准来分拣我们的数据。
