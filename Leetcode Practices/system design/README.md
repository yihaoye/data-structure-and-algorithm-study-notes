...
**如果实际词云数据非海量（中等）的话，可以（冗余）分表，比如虽然基础数据集较大，但是实际用于词云系统的数据记录较小时，也可以把这部分数据单独拿出来部署到独立的 ES 集群，这样可以直接使用 ES 内置功能比如 doc_values（更小的甚至可以使用 fielddata），避免从零实现词云功能（基于纯列式数据库的方案，适用于海量数据、高性能要求）。**  
