...
> 下面说点关于 OOD 的闲话。  
> OOD 刚刚被发明的时候，是被很多人认为是 Silver Bullet 的。大家觉得它从根本上解决了代码 reuse (复用)的问题——低耦合高内聚的代码，使得代码的模块化、重复使用不再复杂，进而解决了软件界一直存在的重造轮子的问题——结果我们都看到了，软件界到今天也仍然在重造轮子。软件界没有银弹—— OOD 不是，SOA 不是，PaaS 不是，IaaS 也不是。说点哲学的，软件的复杂是内秉的：因为人类的需求是基于人类的高阶逻辑的，而计算机至今能满足的是一阶逻辑演算，所以软件界在强AI出现之前不能解决软件的统一解决方案是非常正常的。  
