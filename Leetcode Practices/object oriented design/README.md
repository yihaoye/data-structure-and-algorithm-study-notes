...
* 面向对象设计基础，部分参考[链接](https://www.youtube.com/watch?v=oaUVVBOhyJw)
  ...
  * 设计原则
    ...
    * [IOC（控制反转）](https://www.cnblogs.com/tanghaorong/p/13364634.html) - 传统应用程序都是由开发者在类内部主动创建依赖对象，从而导致类与类之间高耦合，难于测试；通过 IOC 比如依赖注入（又或者使用 IOC 容器并把创建和查找依赖对象的控制权交给它，由容器进行注入组合对象），可以使得对象与对象之间松散耦合，也方便测试，利于功能复用，更重要的是使得程序的整个体系结构变得非常灵活。
