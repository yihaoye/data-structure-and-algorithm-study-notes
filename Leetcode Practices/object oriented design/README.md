...
* 面向对象设计基础，部分参考[链接](https://www.youtube.com/watch?v=oaUVVBOhyJw)
  ...
  * 设计原则
    ...
    * [IOC（控制反转）](https://www.cnblogs.com/tanghaorong/p/13364634.html) - 传统应用程序都是由开发者在类内部主动创建依赖对象，从而导致类与类之间高耦合，难于测试；通过 IOC 比如依赖注入（又或者使用 [IOC 容器比如 Spring](https://liaoxuefeng.com/books/java/spring/ioc/basic/index.html) 并把创建和查找依赖对象的控制权交给它，由容器进行注入组合对象），可以使得对象与对象之间松散耦合，也方便测试，利于功能复用，更重要的是使得程序的整个体系结构变得非常灵活。（该设计理念源于好莱坞选角模式/好莱坞原则 Don't call us, we'll call you，即由制片方/高层主动决定何时调用演员/低层，而非演员主动联系制片方）
      * [DI 依赖注入](./IOC/DependencyInjection.go) - 依赖注入是一种实现控制反转的具体方式，它通过将一个组件的依赖传递给它，而不是让组件自己创建或查找依赖。这通常通过构造函数注入、方法注入或属性注入来实现（基于接口、set 方法、构造函数、注解等等）。依赖注入的目的是将组件解耦，使得组件不需要关心如何获取它所依赖的其他组件，而是通过外部注入的方式获取。
      * DL 依赖查找 - Dependency Lookup，与依赖注入的区别是，依赖查找主动索取相应类型的对象。但因为侵入性大、耦合高，在业务层代码里用得很少，更多出现在框架底层或特殊场景。
    * 复杂性管理
      * 封装：将内部实现细节对外部隐藏，只提供有限的接口供外部使用。
        * 信息隐藏
        * [抽象泄漏（Leaky Abstractions）](../../Quality%20Assurance%20and%20Best%20Practice/降低软件复杂性一般原则和方法.md#抽象泄漏) - 任何试图减少或隐藏复杂性的抽象，其实都并不能完全屏蔽细节；试图被隐藏的复杂细节总是可能会从抽象层级中 “泄漏” 出来
...
![](./a-cheatsheet-for-uml-class-diagrams.png)  
