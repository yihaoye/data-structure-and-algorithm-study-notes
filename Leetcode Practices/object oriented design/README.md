...
* 面向对象设计基础，部分参考[链接](https://www.youtube.com/watch?v=oaUVVBOhyJw)
  ...
  * 设计原则
    ...
    * SOLID（以下链接皆为示例代码）
      ...
      * [O 开闭原则](./SOLID/OpenClose.java): 对象或类应该对扩展开放，对修改封闭。系统新功能应该通过多态增加新模块（接口实现）并调用完成，而不是在系统里都实现这些不同的行为、逻辑、操作。
      ...
      * [D 依赖反转原则](./SOLID/DependencyInversion.java): 高层模块不要依赖具体实现，而是依赖抽象。
    * IOC（控制反转）
