...
public class Solution {
    public int firstMissingPositive(int[] nums) {
        ...
        //    `else if(nums[i] != nums[nums[i]-1]) swap(nums, i, nums[i]-1);` 其实意思就是第 i 轮里的值必定是索引为 nums[i]-1 的元素的正确值且必定不是索引 i 元素里的正确值，此时对比检查索引为 nums[i]-1 的元素的值是否正确（而不是检查索引为 i 的元素值是否正确 - 这一行为在 if 里已经执行了，只是那里如果检测错误不做寻找数组是否存在正确值更正，更正是通过这里被动进行的），不正确则与 nums[i] 互换进行更正，nums[i]-1 不可能大于 nums.length 或小于 0 (前面 skip 了) 所以不会有数组索引错误。
        ...
        //    nums[i] 有可能有 3 种情况获取正确值，即原值就是正确的，或者在之前 (< 现轮 i) 某轮被检测到不正确而更正，或者之后某轮被检测到不正确而更正。如果这 3 次都没被检测出来证明原数组里没有该索引应有的数值，且如果该索引是第 1 个这种情况的索引，那它的索引值+1就是最小缺失正整数。
