...
class Solution {
    ...
    void sa() {
        ...
        for (double t = hi; t > lo; t *= fa) {
            ...
            // 下面这个表达式的含义是，一开始 t 很大，通常 diff 即使为正数（意思是新的解不如之前的解），但是 diff / t 大概率仍然小于 1，所以 Math.log(diff / t) 也小于 1 有可能比随机数小，此时不会执行 swap，即接受新的差解。
            // 但是如果 diff 比 t 的数量级还大（即新的解太差劲了），那么 Math.log(diff / t) 会大于 1，必然大于随机数。此时执行 swap 意味着恢复原状，即不接受差解。
            // 如果 diff 为负数（意思是新的解比之前的解好），则 Math.log(diff / t) 返回 NaN，if 里的比较判断总是返回 false（Java 语法），后面的 swap 不会被执行，即总是接受新的好解。
            // 随着温度降低，t 越来越小（正小数），diff 此时如果为正数（差解）越发不大可能被接受（必须也为正的比 t 还小的小数），从而大概率执行 swap 即越发不太可能接受差解。diff 如果是负数（好解），仍必被接受。
