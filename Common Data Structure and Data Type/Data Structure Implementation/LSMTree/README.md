...
## 范围查询
当执行范围查询时，需要在不同层级的 LSM 树中查找数据，这通常需要较多的磁盘访问。LSM 树通常不使用链表连接叶子节点，因此不能像 B+ 树那样通过链表连续查找下一个节点。  
在 LSM 树中执行范围查询通常需要合并多个不同层级的数据，这可能会导致较多的磁盘读取操作。为了优化范围查询性能，一些 LSM 树实现会采用一些策略，如"Merge-on-Read"，它们会延迟合并操作，直到真正需要执行范围查询。但即使如此，LSM 树的范围查询性能通常不如 B+ 树。  
总之，B+ 树在范围查询方面通常比 LSM 树更高效，因为它们的结构和数据组织方式更适合此类操作。LSM 树更适合用于写入密集型工作负载和大容量数据的存储系统，而 B+ 树更适合需要高效范围查询的工作负载。  
[相关：LSM 树元素迭代/范围查询实现](https://github.com/facebook/rocksdb/wiki/Iterator-Implementation)  
...
# Lucene
全文搜索的索引引擎，在 Elasticsearch 和 Solr 被使用，它参考、使用类似的方法来存储它的关键词词典（但不是纯粹的 LSM 树）。全文索引比键值索引复杂得多，但是基于类似的想法：在搜索查询中，由一个给定的单词，找到提及单词的所有文档（网页，产品描述等）。这也是通过键值结构实现的：其中键是单词（term），值是所有包含该单词的文档的 ID 列表（postings list）。在 Lucene 中，从词语到记录列表的这种映射保存在类似于 SSTable 的有序文件（不可变的 Lucene 段 segment）中，并根据需要在后台执行段合并（segment merging），以及还有内存缓冲区（Memtable 的对应物）。  
