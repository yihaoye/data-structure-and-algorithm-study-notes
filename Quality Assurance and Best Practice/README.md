...
## 软件开发需要知道的法则定律
* 布鲁克斯法则（Brooks’ Law）- 为一个延期的项目增加人力会只会使这个项目更晚。
* 康威定律（Conway’s Law）- 任何组织在设计一个系统（广义的定义）时，都会产生一个设计，其结构是该组织的沟通结构的复制。
* 墨菲定律（Murphy’s Law）- 如果事情有可能出错，那么它就会出错。
* 侯世达定律（Hofstadter’s Law）- 事情总是比你预料的要花费更长的时间，即使你考虑到了侯世达定律。
* 林纳斯定律（Linus’ Law）- 足够多的眼睛，就可让所有问题浮现。
* 古德哈特定律（Goodhart’s Law）- 当一个指标成为目标时，它就不再是一个好的指标。
* 盖尔定律（Gall’s Law）- 一个能工作的复杂系统是从一个能工作的简单系统演变而来的。一个从头开始建立的复杂系统是不会工作。
* 扎温斯基定律（Zawinski’s Law）- 每个程序都试图扩张，直到它可以读取邮件。那些不能扩张的程序会被能扩张的程序所取代。通常用来对臃肿软件的批评。
* 伊格尔森定律（Eagleson’s Law）- 自己的任何代码，如果有 6 个月或更长时间没有看，就好像是别人写的一样。
* 卢巴斯基法则（Lubarsky’s Law）- 就算运用了所有的最佳编程实践、更新以及维护，但总是会有一个更多的错误需要修复。
* 克努特优化原则（Knuth’s Optimization Principle）- 过早优化是万恶之源。
* 伯斯塔尔法则（Postel’s Law）- 在开发一个库、一个 HTTP API、一个类、一个模块或一个函数时，应该提供向后的兼容性，并对接收的参数变得更加灵活。`对于自己输出要严格; 对于他人的输入要灵活`。
* 复杂度守恒定律（Tesler’s Law of Conservation as Complexity）- 每个应用程序都有其固有的复杂性，这些复杂性是不能被消除或隐藏的。相反，它必须被处理，无论是在产品开发中还是在用户互动中。
* 奥卡姆剃刀原理
...
* 外部依赖/下游异常
  ...
  * 本地缓存或延迟重试
  * 启用降级路径或 mock 响应
* 代码或配置问题（相对少。如果较多则说明团队的软件开发测试发布管理、流程有较大问题，应该优先关注管理问题），常见原因：新版本 Bug、内存泄漏、逻辑死循环、依赖冲突
  * 回滚版本至故障前或更新到最新补丁版
  * 依赖更新
  * 重启应用进程或容器以释放资源
