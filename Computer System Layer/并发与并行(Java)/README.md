...
选择合适的锁取决于应用程序的需求和设计，以及线程之间的互动。以下是一些常见的锁以及它们在不同情况下的使用建议：
1. **synchronized 关键字**：
   - 适用于简单的同步需求，如方法级别的同步。
   - 方便使用，不需要显式创建锁对象。
   - 可重入，同一个线程可以多次获取相同对象的锁。
   - synchronized 块可以指定锁的粒度，使得更多的代码可以并发执行。
2. **ReentrantLock**：
   - 提供了比 synchronized 更多的高级特性，如可中断锁、定时锁、公平性等。
   - 可以在需要更多控制权和灵活性的情况下使用。
   - 适用于复杂的同步需求，需要显示地创建锁对象。
3. **ReadWriteLock**：
   - 适用于读多写少的场景，其中多个线程可以同时读取数据，但写操作需要互斥。
   - 通过 `ReentrantReadWriteLock` 实现，可以提高并发性。
4. **StampedLock**：
   - 适用于读写操作的并发性需求，它提供了乐观读取和悲观写入的机制。
   - 用于高度竞争的场景，其中读操作频繁，写操作较少。
5. **Semaphore**：
   - 用于控制同时访问某一资源的线程数量。可以用于限流等场景。
   - 允许一定数量的线程同时获取许可，其他线程需要等待。
6. **CountDownLatch** 和 **CyclicBarrier**：
   - 用于协调多个线程的执行。
   - `CountDownLatch` 用于等待一组线程完成，而 `CyclicBarrier` 用于等待一组线程都达到某个屏障点后再继续执行。
7. **LockSupport**：
   - 用于线程的挂起和唤醒，可以用于自定义的同步机制。
   - 谨慎使用，需要手动管理线程的状态。
8. **自定义锁**：
   - 在某些复杂的应用场景中，可能需要实现自定义的锁机制。
   - 这可以根据具体需求来设计，但需要格外小心，确保线程安全和性能。
9. **乐观锁 CAS**
   - 适合性能要求较高的非阻塞的写多场景。
...
![](./AbstractQueuedSynchronizer.jpeg)  
