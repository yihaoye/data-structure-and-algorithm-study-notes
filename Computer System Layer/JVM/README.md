...
### 垃圾回收
一些编程语言要求程序员手动管理内存，负责分配和释放内存。比如 C 和 C++（[C 代码示例](../mem_free.c)），程序员必须使用 `malloc`、`free`、`new` 和 `delete` 等命令来分配和释放内存。如果程序员忘记释放不再使用的内存，就会导致内存泄漏，最终可能耗尽系统内存（后续分配时报错）。相反，如果程序员错误地释放了仍在使用的内存，就会导致悬挂指针，可能引发程序崩溃或不可预测的行为。  
  
Java 采用自动垃圾回收机制，程序员不需要手动管理内存。  
...
* [垃圾回收机制中，引用计数法是如何维护所有对象引用的？- Gityuan 的回答](https://www.zhihu.com/question/21539353/answer/95667088)  
