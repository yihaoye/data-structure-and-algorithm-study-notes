...
* 第一范式：要求数据库的表的字段（列）必须是原子值（即不可再分割的原子数据项，不能是集合，数组，记录等非原子数据项，否则必须拆分为不同的字段）。一些多值数据类型比如 JSONB 只要其数据本身是一个逻辑整体、有内在联系的也是可以的（[Ref](https://stackoverflow.com/questions/74165358/storing-json-in-a-database-violates-1nf)）。
