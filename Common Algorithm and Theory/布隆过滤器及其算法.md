...
其他应用场景：LSM 树实现与优化、网页 URL 去重、垃圾邮件识别、大集合中重复元素的判断和[缓存穿透](./../Computer%20System%20Layer/数据库/Redis/README.md#缓存三大故障)（利用布隆过滤器减少磁盘 IO 或者网络请求，因为一旦一个值必定不存在的话，可以不用进行后续昂贵的查询请求）等问题。  
