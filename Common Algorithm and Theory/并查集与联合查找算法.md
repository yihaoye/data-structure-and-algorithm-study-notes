...
**`简单来说其实就是尝试建立一个多叉树（即没有头尾环的图亦即如树般有一个根结点)或路径压缩后的无向图（有环）`(有些题目本身给出关系无向也可以转换为有向比如 LC Q261，而且如果图必须有向则只关心是否有贪吃蛇那样的头尾环，对于 C -> B -> A 而 B -> A 这种忽略因为 C 会被路径压缩为 C -> A 但是这种忽略不适用于无向场景要求比如 LC Q684；若无环，则理论上同一集合内的结点/元素经过足够多次合并操作/或称路径压缩后，原本较深的多叉树会最终简化为一个只有两层的多叉树 -- 只有一个根结点和一层叶子结点；[若有环，则理论上同一集合内的结点/元素经过足够多次合并操作/或称路径压缩后，原本较深的图会最终会简化为一个只有一层的状态机/或有环图结构](./../Leetcode%20Practices/algorithms/medium/684%20Redundant%20Connection.java)）。**  
