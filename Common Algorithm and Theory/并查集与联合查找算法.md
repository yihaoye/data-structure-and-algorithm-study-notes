...
并查集被很多 OIer 认为是最简洁而优雅的数据结构之一，主要用于解决一些元素分组的问题。它管理一系列不相交的集合，并支持两种操作：合并与查找。  
并查集（Disjoint-Set/Union-Find Forest）是一种数据结构，`简单来说其实就是尝试建立一个多叉树并检查是否有环（总是有向环检查，即使题目本身给出关系无向也可以转换为有向比如 LC Q261，而且只关心是否有贪吃蛇那样的头尾环，对于 C -> B -> A 而 B -> A 这种不关心因为 C 会被路径压缩为 C -> A）；若无环，则理论上同一集合内的结点/元素经过足够多次合并操作/或称路径压缩后，原本较深的多叉树会最终简化为一个只有两层的多叉树 -- 只有根结点和叶子结点；若有环，则理论上同一集合内的结点/元素经过足够多次合并操作/或称路径压缩后，原本较深的图会最终会简化为一个只有一层的状态机/或有向图结构`。联合查找算法是基于并查集这一数据结构来执行、使用，其原理与功能如下：  
