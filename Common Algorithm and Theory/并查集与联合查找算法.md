...
`简单来说其实就是尝试建立一个多叉树（即没有头尾环的图亦即如树般有一个根结点，有些题目本身给出关系无向也可以转换为有向比如 LC Q261，而且如果图必须有向则只关心是否有贪吃蛇那样的头尾环，对于 C -> B -> A 而 B -> A 这种忽略因为 C 会被路径压缩为 C -> A 但是这种忽略不适用于无向场景要求比如 LC Q684；若无环，则理论上同一集合内的结点/元素经过足够多次合并操作/或称路径压缩后，原本较深的多叉树会最终简化为一个只有两层的多叉树 -- 只有一个根结点和一层叶子结点；若有环，则理论上同一集合内的结点/元素经过足够多次合并操作/或称路径压缩后，原本较深的图会最终会简化为一个只有一层的状态机/或有环图结构）。`联合查找算法是基于并查集这一数据结构来执行、使用的，其原理与功能如下：  
...
class UnionFindSet {
  ...
  public int Find(int u) { // 查询（Find）：递归写法，与上面写法不同的是一次 Find 调用会将该节点所属树/路径的全部节点都 path compression，且会将输入节点的 parents_[x] 完成赋值为根父节点
