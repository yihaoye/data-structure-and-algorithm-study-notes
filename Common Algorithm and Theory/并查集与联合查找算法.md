...
* 从两个子节点中挑一个的并查集复制（N1 个元素）
* 将另一个元素的并查集里的全部 N2 个元素遍历添加进这个复制并查集即可（该复制并查集即当前父节点的并查集），添加的过程比如根据当前 _node 找到相关 _root（递归 Find 因为需要完全路径压缩），然后在另一个并查集 dsu2.Find(_root) 看是否根节点是自己，如果是或者不存在 _root 则把 <_node, _root> 加进去 Map，否则把 <_node, dsu2.Find(_root)> 加进去
