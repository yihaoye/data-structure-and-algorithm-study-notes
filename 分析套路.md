...
Match | Regular Expression <br/>Find Word in XXX |  | DP、递归、字典树、哈希表、滚动哈希、KMP、队列 | Q10 Q720 Q49 Q792
(Fixed)Range (Optimal) Value/Area | Last/Next (Max/Min/Bigger/Smaller) Number/Sum/DiffSum <br/>蓄水/积水面积(二维或三维等等) | 输入可以是：<br/>树节点 <br/>线性表 <br/>Matrix/矩阵 | 双指针/滑动窗口和贪心、DP、一维前缀和/二维前缀和、<br/>差分数组、递归、单调栈 | Q11 Q221 Q42 Q304 Q124 Q437  Q239 Q2104 Q503 Q64 Q362
...
集合区分/统计/划分/分割 |  | 输入可以是：<br/>树节点 <br/>图节点 <br/>Matrix/矩阵 | 并查集、BFS+染色+自定义对象 | 
Design a XXX | (Basic Design) <br/>编程语言内置存在的数据结构或类 |  | 考虑以下数据结构的组合使用：数组、ArrayList、LinkedList、自定义对象、指针或引用 | Q155 Q706 Q707
| | (Advanced Design) <br/>不常用/不通用的或编程语言内置里没有的数据结构或类(Twitter) |  | 考虑各种算法并以下数据结构的组合使用：数组、ArrayList、（单向或双向）LinkedList、自定义对象、指针或引用、哈希表、红黑树、哈希集、队列、堆/优先队列、栈、甚至语言内置工具类如 StringBuilder 等等 | Q146 Q208 Q355 Q432 Q2296 Q362 Q1381
...
Dependency Conflict/Schedule | Course Schedule |  | 拓扑排序、哈希表(集)+遍历、贪心、链表/树/图 | Q207 Q210 Q1462 Q582 Q2328
...
(Range) Update and (Range) Query/Calculation |  |  | 树状数组、线段树、懒更新 | Q307 Q1381
