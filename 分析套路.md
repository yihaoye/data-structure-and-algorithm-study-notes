...
贪心可以的动规一定可以，动规可以的贪心不一定可以；贪心经常可以通过 PriorityQueue 实现；树状数组 vs 线段树: 能写树状数组的先写树状数组，但是树状数组可以的线段树一定可以，线段树可以的树状数组不一定可以；元素不重复的情况下，TreeSet 比 PriorityQueue 好用些，元素重复的话只能用 PriorityQueue（其实仍然可以通过 TreeMap 替代）。  
