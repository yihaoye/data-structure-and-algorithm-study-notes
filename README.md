...
1. 思考一（重要），发生在面试的最开始、影响整场面试体验，先理清题目是关于哪（几）个 Topic，比如什么类型数据结构、算法相关（如队列、堆栈、树、字符串、布尔或二进制、数组、图，如回溯法、动态规划、贪婪算法、二分搜索、二进制运算、迭代递归、模拟、排序、遍历、前缀和与差分、单调栈、并查集、数学、设计），通常若题目没有明显处理算法暗示或思绪提供时则可以考虑暴力破解（如遍历、回溯法、递归循环穷举法或 DFS、BFS 等），又或通过题目性能要求推测考点算法 - 比如要求性能达到 logN 时即很可能要使用二分搜索算法。（记住递归可以顺序也可以反序 - 如面试金典 9.2-2.5 进阶）。通常只要推测出 Tag、Topic 就可极大降低做题的难度。
   1. 套路：根据输入数据判断解题方向
      1. 套路一，输入为一维数组时，可以考虑暴力解（DFS、BFS 以及附加记忆化搜索）、动态规划、双指针、滑动窗口、排序、二分搜索、二进制运算、前缀和与差分、哈希表、贪心、KMP。注意条件：元素是否唯一、是否有序、数组长度。
      2. 套路二，输入为矩阵、二维（比如数组）时，可以考虑暴力解（DFS、BFS 以及附加记忆化搜索）、动态规划、并查集等等。注意条件：值的范围、是否可以修改矩阵、矩阵大小。
      3. 套路三，输入为字符串时，可以考虑暴力解、双指针、滑动窗口、栈、动态规划、贪心、KMP。注意条件：字符集范围、字符串长度。
      4. 套路四，输入为数字时，可以考虑递归、二分查找。注意条件：数字是否整数、数字范围。
      5. 套路五，输入为链表（头节点）时，可以考虑快慢指针、哈希表。注意条件：单向还是双向链表、是否有环、链表长度。
      6. 套路六，输入为树（根节点）时，可以考虑暴力解（DFS - 二叉树可以重点考虑中序遍历、BFS）、递归、栈。注意条件：是否是特殊类型的二叉树、是否平衡、节点数量。例如 LC Q98
      7. 套路七，输入为图（出发节点）时，可以考虑暴力解（DFS、BFS）、并查集、Dijkstra。注意条件：是否有环、是否是简单图、节点值的范围、节点数量。
   2. 套路：根据描述中的关键词（或概念）判断解题方向
      1. 套路一，关键词/概念：Add Up to Target (Closer or Equals)，可以考虑哈希表、排序、双指针、回溯（注意时间复杂度要求场景），例如 LC(Leetcode) Q1 Q15 Q39
      2. 套路二，关键词/概念：Longest/Maximum Substring/Subarray/Sublist/Subset，可以考虑滑动窗口，例如 LC Q3 Q5 Q53
      3. 套路三，关键词/概念：Median，可以考虑自平衡二叉树、堆（优先队列），例如 LC Q4
      4. 套路四，关键词/概念：Sum，可以考虑模拟算法+进位符（标记变量）、前缀和，例如 LC Q2
      5. 套路五，关键词/概念：Palindromic（回文），可以考虑栈、双指针，例如 LC Q20
      6. 套路六，关键词/概念：Regular Expression，可以考虑 DP、递归，例如 LC Q10
      7. 套路七，关键词/概念：Range Max/Min，可以考虑双指针和贪婪、DP，例如 LC Q11 Q55?
      8. 套路八，关键词/概念：All Possible Combinations，可以考虑暴力解（递归或迭代）+剪枝、回溯法、DP（记忆化搜索），例如 LC Q17 Q2266 Q39 Q46 Q62 Q70
      9. 套路九，关键词/概念：Merge XXX (Data Structure)，可以考虑模拟、递归、多路归并（最小堆）、排序，例如 LC Q21 Q23 Q56
      10. 套路十，关键词/概念：Search/Find in Sorted XXX (Data Structure)，可以考虑二分搜索，例如 LC Q33 Q34
      11. 套路十一，关键词/概念：Find Word in XXX，可以考虑字典树、哈希表，例如 LC Q720 Q49
      12. 套路十二，关键词/概念：Range Sum/Diff，可以考虑双指针、动态规划、前缀和（一维、二维），例如 LC Q42 Q304
      13. 套路十三，关键词/概念：Rotate Operation，可以考虑模拟算法，例如 LC Q48 Q189
      14. 套路十四，关键词/概念：String/Word Operation，可以考虑 DP、暴力解（递归）+剪枝，例如 LC Q72
      15. 套路十五，关键词/概念：Traversal，可以考虑暴力解（DFS、BFS），例如 LC Q94 Q102 Q104 Q105
