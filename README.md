...
1. 思考一（重要），发生在面试的最开始、影响整场面试体验，先理清题目是关于哪（几）个 Topic，比如什么类型数据结构、算法相关（如队列、堆栈、树、字符串、布尔或二进制、数组、图，如回溯法、动态规划、贪婪算法、二分搜索、二进制运算、迭代递归、模拟、排序、遍历、前缀和与差分、单调栈、并查集、数学、设计），通常若题目没有明显处理算法暗示或思绪提供时则可以考虑暴力破解（如遍历、回溯法、递归循环穷举法或 DFS、BFS 等），又或通过题目性能要求推测考点算法 - 比如要求性能达到 logN 时即很可能要使用二分搜索算法。（记住递归可以顺序也可以反序 - 如面试金典 9.2-2.5 进阶）。通常只要推测出 Tag、Topic 就可极大降低做题的难度。
   1. 套路：根据输入数据判断解题方向
      ...
      3. 输入为字符串时，可以考虑暴力解、双指针、滑动窗口、栈、动态规划、贪心、KMP。注意条件：字符集范围、字符串长度。
   ...
   2. 套路：根据描述中的关键词（或概念）判断解题方向
      1. 关键词/概念：Add Up to Target - Closer-or-Equals，可以考虑哈希表、排序、双指针、回溯（注意时间复杂度要求场景），LC(Leetcode) 例如 Q1 Q15 Q39
      2. 关键词/概念：Longest/Maximum Meet-Requirement Substring/Subarray/Sublist/Subset，可以考虑 DP、双指针（滑动窗口），LC 例如 Q3 Q5 Q53 Q152 Q300
      3. 关键词/概念：Find Median，可以考虑自平衡二叉树（TreeMap）、堆（优先队列），LC 例如 Q4 Q295
      4. 关键词/概念：Sum，可以考虑模拟算法+进位符（标记变量）、前缀和，LC 例如 Q2
      5. 关键词/概念：Palindromic - 回文，可以考虑栈、双指针，LC 例如 Q20 Q131
      6. 关键词/概念：All Possible Combinations，可以考虑暴力解（递归或迭代）+剪枝、回溯法、DP（记忆化搜索），LC 例如 Q17 Q2266 Q39 Q46 Q62 Q70
      7. 关键词/概念：Merge Multiple XXX/Data-Structure，可以考虑模拟、递归、多路归并（最小堆）、排序，LC 例如 Q21 Q23 Q56
      8.  关键词/概念：Find/Search in Sorted/Unsorted XXX/Data-Structure，可以考虑二分搜索、位运算（如果是 Find Digit），LC 例如 Q33 Q34 Q136 Q287 Q240
      9.  关键词/概念：Regular Expression / Match / Find Word in XXX，可以考虑 DP、递归、字典树、哈希表，LC 例如 Q10 Q720 Q49
      10. 关键词/概念：Range Max/Min，可以考虑双指针和贪婪、DP，LC 例如 Q11 Q55?
      11. 关键词/概念：Range (Max/Min/Sum) Sum/Diff，可以考虑双指针、DP、前缀和（一维、二维）、递归，LC 例如 Q42 Q304 Q124
      12. 关键词/概念：Ordered (Max/Min/Sum) Sum/Diff，可以考虑 DP，LC 例如 Q121 Q123（买卖股票系列）
      13. 关键词/概念：Rotate Operation，可以考虑模拟算法、排序，LC 例如 Q48 Q189
      14. 关键词/概念：String/Word Operation，可以考虑 DP、暴力解（递归）+剪枝，LC 例如 Q72
      15. 关键词/概念：Traversal，可以考虑暴力解（DFS、BFS），LC 例如 Q94 Q102 Q104 Q105
      16. 关键词/概念：Copy/Clone，可以考虑哈希表、指针或引用，LC 例如 Q138
      17. 关键词/概念：Digit/Number Operation/Process，可以考虑位运算，LC 例如 Q751
