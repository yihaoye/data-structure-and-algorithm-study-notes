...
3. 编码三，注意一些语言的相似数据结构的不同效率以及 performance，如 Java 的普通 Array 的时间及空间性能都会比 ArrayList 好很多。另外也注意某些语法糖的效率可能较原写法明显降低，如 Java 8 用一些 lambda 写法就会慢很多（如 2ms 变成 90ms），尽管使用的算法本质是一样的，又如 Java 8 stream 流操作（声明式编程）性能可能比不上原来的命令式编程。
