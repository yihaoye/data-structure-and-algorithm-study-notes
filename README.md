...
10. 思考十，若循环重复对一组数据进行类似的重组、操作、改动（改动是下一轮重复类似重组的原因）（比如在每一次循环里对集合排序、使用数据并更改原集合里的某些数据），通常会使得性能受较大影响；可以考虑一下先扩展数据至所有可能性（把所有改动后的可能结果并集），然后再进行一次且仅一次的操作，以空间换时间（例如 [多处理器最小处理时间](./Other%20Practices/04-11-2021%20multiple%20processor%20schedule%20min%20time/Solution.java)）。  
